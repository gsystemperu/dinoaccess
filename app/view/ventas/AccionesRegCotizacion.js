Ext.define('dinoaccess.view.ventas.AccionesRegCotizacion', {
  ...
  onClickBuscarProducto: function (btn) {
    if (Ext.ComponentQuery.query('#cboDatosCliente')[0].getValue()) {
       w = Ext.create('dinoaccess.view.ventas.BuscarProducto', {
        cliente: Ext.ComponentQuery.query('#cboDatosCliente')[0].getValue()
      });
      w.show(btn, function () {}, this);
    } else {
      Ext.Msg.alert("Aviso", "Buscar al cliente para buscar los precios de los productos !!");
      return false;
    }
  },
  ...
  onCalcularTotalVenta: function (conigv) {
   ...
   store = Ext.ComponentQuery.query('#dgvDetalleVenta')[0].getStore();
   t = 0;
   _igv = 0;
  ...
  onClickRowProducto: function (obj, td, cellIndex, record, tr, rowIndex, e, eOpts) {
    ...
    */
    me = this;
    var st = Ext.ComponentQuery.query('#dgvDetalleVenta')[0].getStore();
    var prcio = 0;
    d = {
    ...
    if (st.findRecord('idprod', parseInt(record.get('id')))) {
      Ext.Msg.alert("Error", "Producto ya se encuentra cargada");
      return false;
    }
    st.insert(0, d);
  ...
  onKeyPressTextoDeBusquedaProducto: function (texto, e, eOpts) {
    if (e.charCode == 13) {
      this.onClickBuscarProductoCodigo();
    }
  },
  ...
  onClickBuscarCotizacionesAnteriores: function (btn) {
    ...
    if (_codcliente)
      Ext.create('dinoaccess.view.ventas.CotizacionesClienteBuscar', {
        codigo: _codcliente
      });
