...
folder are mapped to the application's root namespace, 'dinoaccess'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "dinoaccess"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'dinoaccess' in the same way
as `"dinoaccess/sass/src"`.
